...
// Comme countByTag, mais renvoie les contacts eux-mêmes (id + tags) au lieu
// d'un simple compte. Utilisé par le webhook Stripe pour retrouver TOUS les
// membres d'une équipe (tagués "equipe-<teamId>" à l'inscription) et les
// taguer un par un comme payés, puisqu'un paiement d'équipe ne concerne
// qu'un seul email côté Stripe.
export async function searchContactsByTag(tag) {
  let results = [];
  let searchAfter = undefined;
  let pages = 0;
